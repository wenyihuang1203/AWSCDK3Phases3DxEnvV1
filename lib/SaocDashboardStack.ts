...
interface SaocDashboardStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocDashboardStack extends Stack {
	constructor(scope: App, id: string, props: SaocDashboardStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInDashboard', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInDashboard', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInDashboard', {
		...
		//===========================================================//
		//=== 3DDashboard Install scripts File as asset to execute ===//
		//===========================================================//
		const assetDashboard = new Asset(this, 'AssetDashboard', {
			path: './documents/dashboardInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetDashboard.bucket,
			bucketKey: assetDashboard.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetDashboard.grantRead(instanceRole);
		...
		const SaocDashboardInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocDashboardInstance).add('Name', props.TagName);
		Tags.of(SaocDashboardInstance).add('Type', props.TagType);
		Tags.of(SaocDashboardInstance).add('Stage', 'Build');
		Tags.of(SaocDashboardInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocDashboardInstance).add('Service', '3DDashboard');
