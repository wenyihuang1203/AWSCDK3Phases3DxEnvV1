...
interface SaocSwymStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocSwymStack extends Stack {
	constructor(scope: App, id: string, props: SaocSwymStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInSwym', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInSwym', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInSwym', {
		...
		//===========================================================//
		//=== 3DSwym Install scripts File as asset to execute     ===//
		//===========================================================//
		const assetSwym = new Asset(this, 'AssetSwym', {
			path: './documents/swymInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetSwym.bucket,
			bucketKey: assetSwym.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetSwym.grantRead(instanceRole);
		...
		const SaocSwymInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocSwymInstance).add('Name', props.TagName);
		Tags.of(SaocSwymInstance).add('Type', props.TagType);
		Tags.of(SaocSwymInstance).add('Stage', 'Build');
		Tags.of(SaocSwymInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocSwymInstance).add('Service', '3DSwym');
