...
interface SaocFtsStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocFtsStack extends Stack {
	constructor(scope: App, id: string, props: SaocFtsStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInFts', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInFts', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInFts', {
		...
		//===========================================================//
		//=== 3DFts Install scripts File as asset to execute ===//
		//===========================================================//
		const assetFts = new Asset(this, 'AssetFts', {
			path: './documents/ftsInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetFts.bucket,
			bucketKey: assetFts.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetFts.grantRead(instanceRole);
		...
		const SaocFtsInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocFtsInstance).add('Name', props.TagName);
		Tags.of(SaocFtsInstance).add('Type', props.TagType);
		Tags.of(SaocFtsInstance).add('Stage', 'Build');
		Tags.of(SaocFtsInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocFtsInstance).add('Service', '3DSpaceIndex');
