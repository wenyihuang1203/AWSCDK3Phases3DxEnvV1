...
interface SaocFederatedStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocFederatedStack extends Stack {
	constructor(scope: App, id: string, props: SaocFederatedStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInFederated', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInFederated', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInFederated', {
		...
		//===========================================================//
		//=== 3DFederated Install scripts File as asset to execute ===//
		//===========================================================//
		const assetFederated = new Asset(this, 'AssetFederated', {
			path: './documents/federatedInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetFederated.bucket,
			bucketKey: assetFederated.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetFederated.grantRead(instanceRole);
		...
		const SaocFederatedInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocFederatedInstance).add('Name', props.TagName);
		Tags.of(SaocFederatedInstance).add('Type', props.TagType);
		Tags.of(SaocFederatedInstance).add('Stage', 'Build');
		Tags.of(SaocFederatedInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocFederatedInstance).add('Service', 'FedSearch');
