...
interface Saoc3DIndexStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class Saoc3DIndexStack extends Stack {
	constructor(scope: App, id: string, props: Saoc3DIndexStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonIn3DIndex', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeIn3DIndex', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolIn3DIndex', {
		...
		//===========================================================//
		//=== 3D3DIndex Install scripts File as asset to execute ===//
		//===========================================================//
		const asset3DIndex = new Asset(this, 'Asset3DIndex', {
			path: './documents/threeDIndexInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: asset3DIndex.bucket,
			bucketKey: asset3DIndex.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		asset3DIndex.grantRead(instanceRole);
		...
		const Saoc3DIndexInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(Saoc3DIndexInstance).add('Name', props.TagName);
		Tags.of(Saoc3DIndexInstance).add('Type', props.TagType);
		Tags.of(Saoc3DIndexInstance).add('Stage', 'Build');
		Tags.of(Saoc3DIndexInstance).add('Platform', '3DExperience R2022x');
		Tags.of(Saoc3DIndexInstance).add('Service', '3DIndex');
