...
interface SaocNotificationStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocNotificationStack extends Stack {
	constructor(scope: App, id: string, props: SaocNotificationStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInNotification', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInNotification', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInNotification', {
		...
		//===========================================================//
		//=== 3DNotification Install scripts File as asset to execute ===//
		//===========================================================//
		const assetNotification = new Asset(this, 'AssetNotification', {
			path: './documents/NotificationInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetNotification.bucket,
			bucketKey: assetNotification.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetNotification.grantRead(instanceRole);
		...
		const SaocNotificationInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocNotificationInstance).add('Name', props.TagName);
		Tags.of(SaocNotificationInstance).add('Type', props.TagType);
		Tags.of(SaocNotificationInstance).add('Stage', 'Build');
		Tags.of(SaocNotificationInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocNotificationInstance).add('Service', '3DNotification');
