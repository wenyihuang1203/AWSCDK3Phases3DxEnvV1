...
interface SaocOrchStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocOrchStack extends Stack {
	constructor(scope: App, id: string, props: SaocOrchStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInOrch', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInOrch', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInOrch', {
		...
		//===========================================================//
		//=== 3DOrch Install scripts File as asset to execute ===//
		//===========================================================//
		const assetOrch = new Asset(this, 'AssetOrch', {
			path: './documents/orchInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetOrch.bucket,
			bucketKey: assetOrch.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetOrch.grantRead(instanceRole);
		...
		const SaocOrchInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocOrchInstance).add('Name', props.TagName);
		Tags.of(SaocOrchInstance).add('Type', props.TagType);
		Tags.of(SaocOrchInstance).add('Stage', 'Build');
		Tags.of(SaocOrchInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocOrchInstance).add('Service', '3DOrchestrate');
