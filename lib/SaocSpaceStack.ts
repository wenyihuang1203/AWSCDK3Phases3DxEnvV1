...
interface SaocSpaceStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocSpaceStack extends Stack {
	constructor(scope: App, id: string, props: SaocSpaceStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInSpace', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInSpace', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInSpace', {
		...
		//===========================================================//
		//=== 3DSpace Install scripts File as asset to execute ===//
		//===========================================================//
		const assetSpace = new Asset(this, 'AssetSpace', {
			path: './documents/spaceInstallFB.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetSpace.bucket,
			bucketKey: assetSpace.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetSpace.grantRead(instanceRole);
		...
		const SaocSpaceInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocSpaceInstance).add('Name', props.TagName);
		Tags.of(SaocSpaceInstance).add('Type', props.TagType);
		Tags.of(SaocSpaceInstance).add('Stage', 'Build');
		Tags.of(SaocSpaceInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocSpaceInstance).add('Service', '3DSpace');
