...
interface SaocFcsStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocFcsStack extends Stack {
	constructor(scope: App, id: string, props: SaocFcsStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInFcs', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInFcs', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInFcs', {
		...
		//===========================================================//
		//=== 3DFcs Install scripts File as asset to execute ===//
		//===========================================================//
		const assetFcs = new Asset(this, 'AssetFcs', {
			path: './documents/fcsInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetFcs.bucket,
			bucketKey: assetFcs.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetFcs.grantRead(instanceRole);
		...
		const SaocFcsInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocFcsInstance).add('Name', props.TagName);
		Tags.of(SaocFcsInstance).add('Type', props.TagType);
		Tags.of(SaocFcsInstance).add('Stage', 'Build');
		Tags.of(SaocFcsInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocFcsInstance).add('Service', 'FCS');
