...
interface SaocPassportStackProps extends StackProps {
	env: Environment,
	vpc: IVpc,
...
export class SaocPassportStack extends Stack {
	constructor(scope: App, id: string, props: SaocPassportStackProps) {
		...
		const asset = new Asset(this, 'AssetCommonInPassport', {
		...
		//===============================================================//
		const assetJavaTomee = new Asset(this, 'AssetJavaTomeeInPassport', {
		...
		//=======================================================//
		const assetDBTool = new Asset(this, 'AssetDBToolInPassport', {
		...
		//===========================================================//
		//=== 3DPassport Install scripts File as asset to execute ===//
		//===========================================================//
		const assetPassport = new Asset(this, 'AssetPassport', {
			path: './documents/passportInstall.sh'
		});
		const localPath4 = multipartUserData.addS3DownloadCommand({
			bucket: assetPassport.bucket,
			bucketKey: assetPassport.s3ObjectKey,
			region: 'us-west-2', // Optional
		});
		multipartUserData.addExecuteFileCommand({ filePath: localPath4 });
		assetPassport.grantRead(instanceRole);
		...
		const SaocPassportInstance = new ec2.Instance(this, props.InstanceName, {
			vpc: props.vpc,
		...
		Tags.of(SaocPassportInstance).add('Name', props.TagName);
		Tags.of(SaocPassportInstance).add('Type', props.TagType);
		Tags.of(SaocPassportInstance).add('Stage', 'Build');
		Tags.of(SaocPassportInstance).add('Platform', '3DExperience R2022x');
		Tags.of(SaocPassportInstance).add('Service', '3DPassport');
